...
export class CreateInterestRateComponent implements OnInit {
  ...
  createInterestRate() {
    ...
    this.interestRateForm = this.formBuilder.group({
      ...
      'currentRate': [this.decimalPipe.transform(currentRate, '1.8-8', locale), [Validators.required, Validators.pattern('^[0-9,\\.]+$'), Validators.min(0), Validators.maxLength(11), Validators.max(100)]],
